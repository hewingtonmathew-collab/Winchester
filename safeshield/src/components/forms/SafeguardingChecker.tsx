...
function RiskBadge({ score }: { score: number }) {
  if (score >= 75) return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-green-500/10 border border-green-500/25 text-green-400"><CheckCircle2 size={13} />Low Risk</span>;
  if (score >= 45) return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-amber-500/10 border border-amber-500/25 text-amber-400"><AlertTriangle size={13} />Medium Risk</span>;
  return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-red-500/10 border border-red-500/25 text-red-400"><XCircle size={13} />High Risk</span>;
}
...
export default function SafeguardingChecker() {
  ...
  const gaps = questions.filter((q) => (answers[q.id] ?? null) !== "yes");
  ...
  if (submitted) {
    return (
      <div className="flex flex-col gap-5">
            ...
            <div className="relative w-28 h-28">
              <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                ...
                <circle cx="50" cy="50" r="40" fill="none"
                  stroke={score >= 75 ? "#22c55e" : score >= 45 ? "#f59e0b" : "#ef4444"}
        ...
        {gaps.length > 0 && (
          <GlassCard>
            <h3 className="text-white font-semibold text-sm mb-4 uppercase tracking-wider">Priority Actions ({gaps.length})</h3>
            <div className="flex flex-col gap-2.5">
              {gaps.sort((a, b) => b.weight - a.weight).map((q) => (
                <div key={q.id} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                  <span className={`mt-0.5 shrink-0 text-xs font-bold px-1.5 py-0.5 rounded ${answers[q.id] === "partial" ? "bg-amber-500/15 text-amber-400" : "bg-red-500/15 text-red-400"}`}>
                    {answers[q.id] === "partial" ? "PARTIAL" : "GAP"}
                  </span>
                  <div>
        ...
        <button onClick={() => { setSubmitted(false); setAnswers({}); }} className="self-start text-[#38BDF8] text-sm hover:text-white transition-colors">
          ← Start again
        </button>
      </div>
    );
